...
// ---------------------------------------------------------------------------
// Channel Compare/Capture Interrupt & Flag
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Output Compare / PWM Channel Configuration
// ---------------------------------------------------------------------------
...
// Advanced Timers (TIM1 / TIM8) Main Output Enable (BDTR->MOE)
...
inline void configureInputCapture(TIM_TypeDef* timer, Channel ch, CaptureEdge edge, InputPrescaler psc) noexcept
{
    ...
    if(ch == Channel::CH1)
    {
        // CC1S = 01 (TI1 mapped to IC1), IC1PSC = pscVal
