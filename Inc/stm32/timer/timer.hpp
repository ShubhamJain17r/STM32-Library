...
public:
    ...
    /// @brief Construct timer at a specific interrupt frequency in Hz
    explicit TimerHandler(std::uint32_t frequencyHz);

    /// @brief Construct timer with explicit prescaler and auto-reload values
    TimerHandler(std::uint32_t prescaler, std::uint32_t autoReload);

    /// @brief Set the timer update frequency in Hz (reconfigures PSC and ARR)
    void setFrequency(std::uint32_t frequencyHz) noexcept;

    /// @brief Set the timer period in microseconds (1 us resolution, requires TIM_CLK >= 1 MHz)
    ...
    /// @brief Enable or disable One-Pulse Mode (timer stops after one update event)
    void setOnePulseMode(bool enable) noexcept;

    /// @brief Set the counting direction / alignment mode (not available on Basic timers)
    ...
    /// @brief Blocking microsecond hardware delay using One-Pulse Mode
    void delayUs(std::uint32_t us) noexcept;

    /// @brief Blocking millisecond delay (calls delayUs 1000 times)
...
void TimerHandler<I>::calculatePscArr(std::uint32_t targetFreqHz, std::uint32_t& pscOut, std::uint32_t& arrOut) const noexcept
{
    ...
    {
        // 32-bit timers (TIM2, TIM5): target 1 MHz tick (1 us resolution)
    ...
    // General: find PSC and ARR for 16-bit timers
...
void TimerHandler<I>::setPeriodUs(std::uint32_t periodUs) noexcept
{
    ...
    const std::uint32_t timClk = rcc::getTimerFrequency(Traits<I>::bus);
    const std::uint32_t psc = (timClk / 1'000'000U) - 1U; // 1 us tick
